...
import { useComposedRefs } from '@repo/ui/lib/composeRefs';
import { cn } from '@repo/ui/lib/utils';
import { useAsRef } from '@repo/ui/hooks/useAsRef';
import { useIsomorphicLayoutEffect } from '@repo/ui/hooks/useIsomorphicLayoutEffect';
import { useLazyRef } from '@repo/ui/hooks/useLazyRef';
			...
			if (shouldRecompute && rootRef.current) {
				requestAnimationFrame(() => {
					...
					if (currentState.cropSize && currentState.mediaSize) {
						...
						if (
							Math.abs(newPosition.x - currentState.crop.x) >
								0.001 ||
							Math.abs(newPosition.y - currentState.crop.y) >
								0.001
						) {
...
function CropperImpl(props: CropperImplProps) {
			...
			if (event.touches.length === 2) {
				...
				if (firstTouch && secondTouch) {
					...
					rafPinchTimeoutRef.current = requestAnimationFrame(() => {
						...
						const rotationAngle = getRotationBetweenPoints(
							pointA,
							pointB,
						);
						const rotationDiff =
							rotationAngle - lastPinchRotationRef.current;
	...
	const onGestureChange = React.useCallback(
		(event: GestureEvent) => {
			...
			const newRotation =
				gestureRotationStartRef.current + event.rotation;
			store.setState('rotation', newRotation);
		},
		[onZoomChange, store],
	);
	...
	return (
		<RootPrimitive
			...
			ref={composedRef}
			className={cn(
				`absolute inset-0 flex cursor-move touch-none select-none
				items-center justify-center overflow-hidden outline-none`,
				className,
			)}
			onKeyUp={onKeyUp}
			onKeyDown={onKeyDown}
...
function useMediaComputation<T extends HTMLImageElement | HTMLVideoElement>({
	...
	const computeSizes = React.useCallback(() => {
		...
		const isScaledDown =
			media.offsetWidth < naturalWidth ||
			media.offsetHeight < naturalHeight;
